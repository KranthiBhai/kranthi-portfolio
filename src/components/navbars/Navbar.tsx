    ...
    <nav className=" bg-gray-50 shadow-xl w-full h-20 flex items-center justify-between px-2 sm:px-10">
      <Link to="/">
        <div className=" flex items-center gap-x-2">
          <div>
            <img
              src="/media/home/eco-pic-01.jpg"
              alt="img"
              className=" w-14 h-14 border-2 border-white rounded-lg object-cover"
            />
          </div>
          <div className=" flex flex-col xl:flex-row xl:items-center">
            <h1 className=" sm:text-lg md:text-xl font-bold">
              TATANABOINA JAYA KRANTHI KUMAR
            </h1>
      ...
      <div className="hidden lg:block">
        <NavLink
          style={navLinkStyles}
          className={`${" mx-2 font-bold "} ${currentPage === "home" ? "text-[#C36A2D]" : "text-gray-400 hover:text-[#C36A2D]"}`}
          to="/"
        >
          Home
        </NavLink>
        <NavLink
          style={navLinkStyles}
          className={`${" mx-2 font-bold "} ${currentPage === "resume" ? "text-[#C36A2D]" : "text-gray-400 hover:text-[#C36A2D]"}`}
          to="/resume"
        >
          Resume
        </NavLink>
        <NavLink
          style={navLinkStyles}
          className={`${" mx-2 font-bold "} ${currentPage === "projects" ? "text-[#C36A2D]" : "text-gray-400 hover:text-[#C36A2D]"}`}
          to="/projects"
        >
          Projects
        </NavLink>
        <NavLink
          style={navLinkStyles}
          className={`${" mx-2 font-bold "} ${currentPage === "blog" ? "text-[#C36A2D]" : "text-gray-400 hover:text-[#C36A2D]"}`}
          to="/blog"
        >
          Blog
        </NavLink>
        <NavLink
          style={navLinkStyles}
          className={`${" mx-2 font-bold "} ${currentPage === "contact" ? "text-[#C36A2D]" : "text-gray-400 hover:text-[#C36A2D]"}`}
          to="/contact"
        >
          Contact
        </NavLink>
